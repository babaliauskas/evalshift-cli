...
evalshift all --yes --open                # doctor → run → evaluate → analyze
                                          #   → report, in one command
```

`evalshift all` notices `tools_path` on the prompt and dispatches
through the agent path automatically — every `Call` row in
`raw.jsonl` carries a parsed `ToolTrace`, and the configured
`tool_*` evaluators score against the per-example
`expected_tools` ground truth.

If you'd rather drive each stage by hand (e.g. to re-run just the
evaluation after editing thresholds), the long form still works:
