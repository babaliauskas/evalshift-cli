...
- [x] `ruff check`, `ruff format --check`, `mypy --strict src/evalshift`, `pytest` all green before marking a checkbox done.
...
- [x] Verify v0.1 baseline on the new branch: `pytest`, `ruff check`, `ruff format --check`, `mypy --strict src/evalshift` all green.

---

## Phase 1 — Tool data models + provider parsers + client extension (PRD Week 1)

Goal: `evalshift test-call --model X --tools tools.yaml --prompt "..."` returns a normalised `ToolTrace` for any of the three providers.

### 1.1 Tool data models (`src/evalshift/evaluators/tool_models.py`, new)
...
### 1.2 Provider response parser (`src/evalshift/evaluators/tool_parser.py`, new)
- [x] `ToolParseError(provider, reason, raw)` exception (mirrors existing `ConfigError` shape).
- [x] `detect_provider(model_id) -> str` mapping LiteLLM canonical ids to `anthropic` / `openai` / `gemini`. Reuse the prefix logic from `evalshift.models.registry._infer_provider_and_canonical`.
...
### 1.3 Tool-aware model client (`src/evalshift/models/client.py`, extend)
...
### 1.4 `evalshift test-call --tools` extension (`src/evalshift/cli/commands/test_call.py`, extend)
...
- [x] `evalshift test-call --model gemini-2.5-flash --tools examples/agent/tools.yaml --prompt "find ACME's recent orders"` returns a Rich panel with the tool names list, no traceback.
...
### 2.1 Suite extension (`src/evalshift/suite/models.py`, extend)
...
- [x] **Backward compatibility test**: existing v0.1 suite JSONL (no tool fields) still loads via `evalshift.suite.loader.load_jsonl`.
...
### 2.2 Config schema extension (`src/evalshift/config/models.py`, extend)
...
### 2.3 Tools loader (`src/evalshift/evaluators/tool_loader.py`, new)
...
### 2.4 `Call` model + persistence extension (`src/evalshift/runner/models.py`, extend)
- [x] Add `trace: ToolTrace | None = None` to `Call`.
- [x] Wire serialisation in `evalshift.runner.checkpoint.append_call` and `iter_calls` — `ToolTrace` round-trips losslessly via pydantic so this is mostly free.
...
### 2.5 Orchestrator dispatch (`src/evalshift/runner/orchestrator.py`, extend)
...
### 2.6 `ToolSelectionEvaluator` (`src/evalshift/evaluators/tool_selection.py`, new)
...
### 2.7 Evaluator runner integration (`src/evalshift/cli/commands/evaluate.py`, extend)
...
- [x] `evalshift.yaml` (1 agent prompt, 3 tools, length structural + tool_selection; Gemini defaults like the v0.1 init).
...
- [x] `evalshift run --from gemini-2.5-flash --to gemini-2.5-pro` against `examples/agent/` (mocked LLM in tests, real Gemini for manual smoke) writes `Call` rows with `.trace` populated; `evalshift evaluate` produces `tool_selection` records; existing report still renders without crashing on the new evaluator name.
...
### 3.1 `ToolArgumentsEvaluator` (`src/evalshift/evaluators/tool_arguments.py`, new)
...
- [x] Per-field strategies: `exact`, `subset` (recursive dict/list), `numeric` (relative-error decay, `numeric_tolerance` clamps), `semantic` (cosine similarity via injected `embeddings_fn` — reuse the embedding helper from `evalshift.evaluators.semantic.CosineSimilarityEvaluator`).
...
### 3.2 `ToolTraceStructureEvaluator` (`src/evalshift/evaluators/tool_trace_structure.py`, new)
...
### 3.3 Wire both new evaluators into `evalshift evaluate` (`src/evalshift/cli/commands/evaluate.py`, extend)
...
### 3.4 `evalshift doctor` warnings (`src/evalshift/cli/commands/doctor.py`, extend)
...
- [x] One test that locks in: bimodal score data (typical of tool evaluators) routes through `evalshift.analysis.statistics.analyze` → Wilcoxon kicks in (Shapiro-Wilk rejects normality), severity classification still works, BH correction still applies.

### Phase 3 verification
- [x] All 7 PRD §2.1 failure modes map to a passing integration test.
- [x] `pytest --cov=evalshift.evaluators.tool_*` ≥ 90%; overall coverage ≥ 94%.
...
### 4.1 Report data extension (`src/evalshift/reports/json.py`, extend)
...
### 4.2 HTML template extension (`src/evalshift/reports/templates/report.html.j2` + `report.css`)
...
- [x] Bump `pyproject.toml` and `src/evalshift/__init__.py` to `0.2.0`.
...
| ------------------ | --------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| Tool data models   | `src/evalshift/evaluators/tool_models.py`                                               | —                                                                                                                                       |
| Provider parsing   | `src/evalshift/evaluators/tool_parser.py`                                               | —                                                                                                                                       |
| Tools loader       | `src/evalshift/evaluators/tool_loader.py`                                               | —                                                                                                                                       |
| New evaluators     | `tool_selection.py`, `tool_arguments.py`, `tool_trace_structure.py`                     | —                                                                                                                                       |
| Model client       | —                                                                                       | `src/evalshift/models/client.py` (+ `complete_with_tools`)                                                                              |
| Suite              | —                                                                                       | `src/evalshift/suite/models.py` (`SuiteExample` + `ExpectedToolCall`)                                                                   |
| Config             | —                                                                                       | `src/evalshift/config/models.py` (`PromptDefinition.tools_path`, three new evaluator configs, `EvaluatorsConfig`)                       |
| Runner             | —                                                                                       | `src/evalshift/runner/models.py` (`Call.trace`), `orchestrator.py` (dispatch), `checkpoint.py` (free, via pydantic round-trip)          |
...
| Examples           | `examples/agent/{evalshift.yaml,prompts.py,tools.yaml,golden.jsonl,README.md}`          | —                                                                                                                                       |
...
- `_StrictModel` pattern (currently parallel in `config/models.py:27` and `runner/models.py:32`) — reuse, extract to `evalshift.utils.pydantic` only if a third copy would land.
- `evalshift.config.loader.ConfigError` / `SuiteError` shape — clone the kind/summary/details + plain/rich rendering pattern for `ToolParseError` and the tool-loader error.
- `evalshift.models.registry.resolve_model` + `_infer_provider_and_canonical` — reuse for provider detection in `tool_parser.py`.
- `evalshift.models.client.ModelClient` retry policy + cost helpers — reuse inside `complete_with_tools`.
- `evalshift.evaluators.base.Evaluator` Protocol + `EvalRecord` + `PairedScore` — every new evaluator implements this.
- `evalshift.evaluators.semantic.CosineSimilarityEvaluator` embedding helper — reuse for the `semantic` strategy in `ToolArgumentsEvaluator`.
- `evalshift.runner.checkpoint.{append_call,iter_calls}` — already round-trips arbitrary pydantic; `Call.trace` Just Works.
- `evalshift.analysis.statistics.analyze` — handles bimodal distributions via the existing Wilcoxon fallback; no changes needed.
- `evalshift.reports.json.build_report_payload` + jinja template structure — extend, don't rewrite.

## Verification (end-to-end)

1. From a fresh checkout: `uv venv --python 3.14 && uv pip install -e .[dev] && pytest` green.
2. `evalshift init` (existing template) still produces a v0.1-compatible project that runs end-to-end.
3. `examples/agent/` runs via `evalshift run` (mocked in tests; real Gemini for the manual smoke) with `tool_selection` / `tool_arguments` / `tool_trace_structure` scores in `scores.jsonl`.
4. `evalshift analyze` produces severity badges on tool evaluators; `evalshift report --open` shows the new "Tool Trace Comparison" section.
...
6. `pytest --cov=evalshift` ≥ 94% (target: don't regress); new modules ≥ 90%.
7. `mypy --strict src/evalshift` clean; `ruff check` + `ruff format --check` clean.
