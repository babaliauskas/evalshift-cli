"""Load and validate ``aimigrate.yaml``.

This module is the single entry point for turning a path on disk into a fully
validated :class:`~aimigrate.config.models.AIMigrateConfig`. Errors raised
...
* **Schema violation**: the YAML parses, but doesn't match
  :class:`AIMigrateConfig`. The pydantic errors are flattened into a list of
...
class ConfigError(Exception):
    """Raised when ``aimigrate.yaml`` is missing, unparseable, or invalid.
...
def load_config(path: str | Path) -> AIMigrateConfig:
    """Load and validate an ``aimigrate.yaml`` file.

    Args:
        path: Path to the YAML file. Accepts either ``str`` or :class:`Path`.

    Returns:
        A fully validated :class:`AIMigrateConfig`.
    ...
    try:
        return AIMigrateConfig.model_validate(data)
