...
defaults:
  source_model: claude-4.5-sonnet
  target_model: claude-5-sonnet
  judge_model: claude-5-sonnet-20260101
  concurrency: 10
  cache: true
  max_cost_usd: 50.0

# Evaluators score the (source, target) outputs.
evaluators:
  # Structural: deterministic checks on output shape.
  structural:
    - type: length
      min_chars: 5
      max_chars: 500
  # Semantic: cosine similarity on embeddings of the two outputs.
  semantic:
    embedding_model: text-embedding-3-small
  # LLM-as-judge: pairwise comparisons. Add multiple judges for multiple
  # criteria; each adds one judge call per (prompt, example).
  llm_judge:
...
# Slices break the suite into subsets analysed separately. The `filter` is
# a Python expression evaluated against an example's `inputs` dict.
slices:
  - name: formal
    filter: "tone == 'formal'"
"""
