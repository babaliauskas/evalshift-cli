...
* :mod:`evalshift_cli.insights.facts` renders every figure the narrative may
  mention into a display string, so the generating model copies rather than
  calculates.
* :mod:`evalshift_cli.insights.templates` turns those same figures into
  deterministic prose — what ships when generation is skipped or fails.
* :mod:`evalshift_cli.insights.generator` prompts a model and validates its
