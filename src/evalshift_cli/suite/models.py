...
class _StrictModel(BaseModel):
    ...
    Mirrors the same-named base in :mod:`evalshift_cli.config.models`. We keep a
...
class SuiteExample(_StrictModel):
    ...
    def _check_tool_expectations_consistent(self) -> Self:
        ...
        * ``toolset_ref == EMPTY_TOOLSET_FINGERPRINT`` -- the ``toolset_ref``
          ...
          :func:`evalshift_cli.captures.reader.load_toolset`'s job, not this
          one's.
        """
        # Deferred import: evalshift_cli.captures (the package __init__) imports
        # captures.models, which imports evalshift_cli.suite.models for
        # SuiteExample -- so a module-level import here would make the first
        # `import evalshift_cli.suite.models` (or anything importing it before
        # evalshift_cli.captures has already been loaded, e.g. the `evalshift`
        ...
        # top-level import here breaks `import evalshift_cli.cli.main` cold.
...
class Suite(_StrictModel):
    ...
    :func:`evalshift_cli.suite.loader.load_jsonl`.
    """
