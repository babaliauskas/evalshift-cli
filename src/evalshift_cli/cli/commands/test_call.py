"""Implementation of ``evalshift test-call``.
...
* You're setting up a new machine and want to confirm your API keys
  work without writing a full ``evalshift.yaml``.
...
from evalshift_cli.evaluators.tool_loader import ToolLoaderError, load_tools
...
def test_call(
    ...
    if meta.display_name.endswith("(passthrough)"):
        console.print(
            f"[dim]→ {meta.id}[/dim]  "
            "[yellow](not in EvalShift registry; passing through to LiteLLM)[/yellow]",
        )
    else:
        console.print(
            f"[dim]→ {meta.id}[/dim]"
...
def _print_result(console: Console, result: CompletionResult) -> None:
    ...
    console.print(
        Panel(body, border_style="green", title="evalshift test-call", title_align="left"),
    )
...
def _print_tool_result(console: Console, result: ToolCompletionResult) -> None:
    ...
    console.print(
        Panel(body, border_style="green", title="evalshift test-call --tools", title_align="left"),
    )
