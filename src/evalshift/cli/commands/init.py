...
a blank directory to a full report in two commands:
...
* ``evalshift.yaml`` — heavily commented config showing every common option.
...
prompts:
  - id: customer_routing
    # detection: how EvalShift finds the prompt body.
    #   manual         - the body is inline below as `content`.
    #   python_string  - EvalShift AST-walks `path` for a module-level
...
def init(
    ...
    if ci:
        files[CI_WORKFLOW_PATH] = _CI_WORKFLOW_TEMPLATE
    ...
    console.print(
        "  [cyan]evalshift all --yes --open[/cyan]"
        " - run the whole pipeline (uses configured Gemini defaults)",
    )
    console.print()
    console.print("  Or drive each stage manually:")
    console.print("    1. [cyan]evalshift doctor[/cyan]     - verify your environment")
    console.print("    2. [cyan]evalshift run --yes[/cyan]  - execute the run")
    console.print("    3. [cyan]evalshift evaluate <run-id>[/cyan]")
    console.print("    4. [cyan]evalshift analyze <run-id>[/cyan]")
    console.print("    5. [cyan]evalshift report <run-id> --open[/cyan]")
    if ci:
        console.print()
        console.print(
