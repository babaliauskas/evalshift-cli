...
class TestRunIdAndHash:
    ...
    def test_run_id_embeds_suite_slug(self) -> None:
        rid = generate_run_id(datetime(2026, 6, 1, tzinfo=UTC), suite_slug="note_search")
        # Date stays first (so runs still sort chronologically), slug in the middle.
        assert rid.startswith("r_20260601_note_search_")
        assert len(rid.split("_")[-1]) == 6

    def test_run_id_slugifies_messy_suite_names(self) -> None:
        rid = generate_run_id(datetime(2026, 6, 1, tzinfo=UTC), suite_slug="My Suite!! v2")
        assert rid.startswith("r_20260601_my_suite_v2_")

    def test_run_id_ignores_empty_slug(self) -> None:
        # A slug that slugifies to nothing falls back to the plain format.
        rid = generate_run_id(datetime(2026, 6, 1, tzinfo=UTC), suite_slug="!!!")
        assert rid.startswith("r_20260601_")
        assert len(rid.split("_")) == 3  # r, date, hex — no empty middle
...
def _push_checkpoint() -> PushCheckpoint:
    return PushCheckpoint(
        client_run_id="r_20260601_abc123",
        server_run_id="0f1d2c3b-4a59-4c8e-9b1f-2d3e4f5a6b7c",
        project_slug="acme/model-migration",
        finalize_url="/runs/0f1d2c3b-4a59-4c8e-9b1f-2d3e4f5a6b7c/finalize",
        view_url="https://app.evalshift.test/runs/0f1d2c3b",
        bundle_sha256="d" * 64,
    )


class TestPushCheckpoint:
    def test_clear_removes_the_directory_the_write_created(self, tmp_path: Path) -> None:
        """``write`` mkdirs, so ``clear`` has to unmake what it made.

        ``push --bundle <path>`` outside a project has no run directory of its
        own: writing the checkpoint creates one purely to hold the resume hint,
        and unlinking only the file would leave an empty ``.evalshift/runs/<id>/``
        behind for a command that never wrote a run.
        """
        directory = run_dir_for("r_20260601_abc123", tmp_path / "runs")
        write_push_checkpoint(directory, _push_checkpoint())
        assert (directory / PUSH_STATE_FILENAME).exists()

        clear_push_checkpoint(directory)

        assert not directory.exists()

    def test_clear_keeps_a_run_directory_that_still_holds_artefacts(self, tmp_path: Path) -> None:
        """A push of a locally-built bundle checkpoints *into* that run's directory.

        Dropping the resume hint must not take ``state.json``/``raw.jsonl``/the
        bundle with it. ``rmdir`` refusing a non-empty directory is the mechanism,
        and this pins that it is load-bearing rather than incidental.
        """
        directory = run_dir_for("r_20260601_abc123", tmp_path / "runs")
        write_state(directory, _state())
        append_call(directory, _call())
        write_push_checkpoint(directory, _push_checkpoint())

        clear_push_checkpoint(directory)

        assert not (directory / PUSH_STATE_FILENAME).exists()
        assert (directory / "state.json").exists()
        assert (directory / "raw.jsonl").exists()
