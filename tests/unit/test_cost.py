...
class TestEstimateHappy:
    ...
    def test_assumed_completion_tokens_uses_primary_default(self) -> None:
        ...
        # Registry default for every model is 4096.
        assert estimate.assumed_completion_tokens == 4096
...
# ---------------------------------------------------------------------------
# Multi-turn history pricing
# ---------------------------------------------------------------------------


class TestPerExampleExtraChars:
    def test_history_raises_estimated_cost_vs_no_history(self) -> None:
        without_history = estimate_run_cost(
            template="Hello {name}",
            examples=[{"name": "Alex"}],
            n_prompts=1,
            models=["gemini/gemini-2.5-flash"],
        )
        with_history = estimate_run_cost(
            template="Hello {name}",
            examples=[{"name": "Alex"}],
            n_prompts=1,
            models=["gemini/gemini-2.5-flash"],
            per_example_extra_chars=[2000],
        )
        assert with_history.avg_prompt_tokens > without_history.avg_prompt_tokens
        assert with_history.estimated_usd > without_history.estimated_usd

    def test_no_extra_chars_matches_prior_behavior(self) -> None:
        # None (the default) and a list of zeros must produce identical
        # estimates — history-free suites keep today's numbers exactly.
        ...
        zeros = estimate_run_cost(
            template="Hi",
            examples=[{}, {}],
            n_prompts=1,
            models=["gemini/gemini-2.5-flash"],
            per_example_extra_chars=[0, 0],
        )
        assert default == zeros

    def test_extra_chars_scale_with_history_size(self) -> None:
        small = estimate_run_cost(
            template="Hi",
            examples=[{}],
            n_prompts=1,
            models=["gemini/gemini-2.5-flash"],
            per_example_extra_chars=[100],
        )
        big = estimate_run_cost(
            template="Hi",
            examples=[{}],
            n_prompts=1,
            models=["gemini/gemini-2.5-flash"],
            per_example_extra_chars=[1000],
        )
        assert big.avg_prompt_tokens > small.avg_prompt_tokens
